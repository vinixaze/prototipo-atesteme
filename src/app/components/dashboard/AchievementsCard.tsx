...
import { rankingSlides } from '../../data/dashboardData';
...
export default function AchievementsCard({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setRankingSlide((prev) => (prev + 1) % rankingSlides.length);
    }, 4000);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4 mb-6 flex-1">
        ...
        >
          <button
            onClick={() => setRankingSlide((prev) => (prev - 1 + rankingSlides.length) % rankingSlides.length)}
          ...
          <button
            onClick={() => setRankingSlide((prev) => (prev + 1) % rankingSlides.length)}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {rankingSlides[rankingSlide] && (
                <>
                  <rankingSlides[rankingSlide].icon
                    className="w-10 h-10 mx-auto mb-2 opacity-60"
                    style={{ color: rankingSlides[rankingSlide].color }}
                  />
                  <p
