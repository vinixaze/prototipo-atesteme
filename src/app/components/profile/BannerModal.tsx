import { X, Lock, Check, Sparkles, RefreshCcw } from 'lucide-react';
import { BannerLevel } from '../../types/profile';

interface BannerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (bannerId: number) => void;
  banners: BannerLevel[];
  selectedBanner: number;
  onNavigateToDigcoins?: () => void;
}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {banners.map((banner) => (
            <button
              ...
              className={`relative p-4 rounded-xl border-2 transition-all duration-300 group ${!banner.unlocked
            ...
            >
              ...
              <div
                className={`w-full h-16 rounded-lg mb-3 flex items-center justify-center bg-gradient-to-r ${banner.gradient} ${!banner.unlocked ? 'grayscale' : ''}`}
              >
