...
export const validateField = (name: string, value: string): string => {
  switch (name) {
    case "nome":
      if (!value.trim()) return "Nome Ç¸ obrigatÇürio";
      if (value.trim().length < 3)
        return "Nome deve ter pelo menos 3 caracteres";
      return "";
    case "email":
    case "emailResponsavel":
      if (!value.trim()) return "E-mail Ç¸ obrigatÇürio";
      if (!validateEmail(value)) return "E-mail invÇ­lido";
      return "";
    case "cpf":
      if (!value.trim()) return "CPF Ç¸ obrigatÇürio";
      if (!validateCPF(value)) return "CPF invÇ­lido";
      return "";
    case "telefone":
    case "telefoneResponsavel": {
      const cleanPhone = value.replace(/\D/g, "");
      if (!cleanPhone) return "Telefone Ç¸ obrigatÇürio";
      if (cleanPhone.length < 10) return "Telefone invÇ­lido";
      return "";
    }
    case "dataNascimento": {
      if (!value.trim()) return "Data de nascimento Ç¸ obrigatÇüria";
      const cleanDate = value.replace(/\D/g, "");
      if (cleanDate.length !== 8) return "Data invÇ­lida";
      const [day, month, year] = value.split("/").map(Number);
      if (!day || !month || !year) return "Data invÇ­lida";
      if (day < 1 || day > 31) return "Dia invÇ­lido";
      if (month < 1 || month > 12) return "MÇ¦s invÇ­lido";
      if (year < 1900 || year > new Date().getFullYear())
        return "Ano invÇ­lido";
      return "";
    }
    case "cep":
      if (value && !validateCEP(value)) return "CEP invÇ­lido";
      return "";
    case "curriculo":
      if (value && !/^https?:\/\/.+/.test(value))
        return "URL invÇ­lida (deve comeÇõar com http:// ou https://)";
      return "";
    default:
      return "";
  }
