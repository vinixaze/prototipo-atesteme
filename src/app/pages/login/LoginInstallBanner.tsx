...
interface LoginInstallBannerProps {
  show: boolean;
  canInstall: boolean;
  onInstall: () => void;
  onDismiss: () => void;
...
export function LoginInstallBanner({ show, canInstall, onInstall, onDismiss }: LoginInstallBannerProps) {
  if (!show) {
    return null;
  }
      ...
      <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            onClick={onInstall}
            disabled={!canInstall}
            className={`px-3 py-1 rounded-lg text-sm font-bold transition-colors whitespace-nowrap ${canInstall
